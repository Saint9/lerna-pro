...
Horizontal.prototype.setStart = function (start) {
  ...
  const part = percentage.from(begin - this.options.min, this.options.max - this.options.min) || 0
  const offset = percentage.of(part, this.slider.offsetWidth - this.handle.offsetWidth)
  const position = (this.options.step) ? findClosest(offset, this.steps) : offset
...
Horizontal.prototype.onmousemove = function (e) {
  ...
  const leftOffset = this.handleOffsetX + e.clientX - this.startX
  const position = (this.steps) ? findClosest(leftOffset, this.steps) : leftOffset
...
Horizontal.prototype.step = function (sliderSize, handleSize) {
  ...
  const part = percentage.from(this.checkStep(this.options.step), this.options.max - this.options.min)
...
Horizontal.prototype.generate = function () {
  ...
  for (const key in elems) {
    if (elems.hasOwnProperty(key)) {
      const temp = this.create(elems[key].type, elems[key].selector)
